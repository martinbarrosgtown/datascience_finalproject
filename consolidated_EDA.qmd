...
ggplot(remit_train, aes(x = deportations + 1, y = remittances + 1)) +
  ...
  theme_minimal()
```
- On a log–log scale, deportations and remittances show a  positive but noisy association, 
suggesting remittances tend to rise with deportations. 
------------------------------------------------------------------------
...
```
- Remittances are most strongly positively correlated with the stock of migrants (0.49),
suggesting migrant presence is a key driver, while other macro variables show only weak associations.
- Remittances as % of GDP are negatively correlated with GDP per capita ( −0.48), 
indicating remittances matter more (relative to GDP) in poorer countries.
------------------------------------------------------------------------
