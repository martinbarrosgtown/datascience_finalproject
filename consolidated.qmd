...
## 10.1 Data Preparation for Modeling
...
Creates a specialized recipe for Ridge and LASSO models. The glmnet engine requires clean data with no infinite values, no missing values, no zero-variance predictors, and normalized scales.

------------------------------------------------------------------------

## 10.3 Model 1: Ridge Regression

Apply L2 regularization to shrink coefficients and reduce overfitting while keeping all predictors in the model.
```{r}
...
Ridge didn't improve performance because the dataset doesn't suffer from severe multicollinearity. The baseline model already generalizes well.

------------------------------------------------------------------------

## 10.4 Model 2: LASSO Regression

The objetive is to apply L1 regularization for both coefficient shrinkage and variable selection. Unlike Ridge, LASSO can set some coefficients to exactly zero, effectively removing predictors.
```{r}
...
The LASSO regularization shows a small set of predictors entering the model as the penalty decreases, highlighting its role as a variable selection method.
...
LASSO found value in regularization. It successfully identified the most predictive subset of features, showing that sparse models work better for this dataset.

------------------------------------------------------------------------

## 10.5 Compare Ridge vs LASSO Performance
```{r}
# RMSE comparison
bind_rows(
...
Direct comparison of Ridge and LASSO at their optimal penalty values. We can see which regularization approach performs better for our data.

------------------------------------------------------------------------

## 10.6 Model 3: Random Forest

Use an ensemble of decision trees to capture non-linear relationships and interactions between predictors without assuming a specific functional form.

Random Forest creates multiple decision trees using random subsets of data and predictors. It uses two key hyperparameters: `mtry` (number of predictors considered at each split) and `min_n` (minimum observations needed to split a node). Final prediction is the average across all trees.
```{r}
library(vip)
...
## Finalize parameters --> check this
rf_params <- rf_wf |> 
...
Uses 5-fold CV instead of 10-fold for computational efficiency (Random Forests are intensive) - Tests 20 different combinations of hyperparameters (trees, mtry, min_n) using maximum entropy design - Selects the best combination based on lowest RMSE - Generates a variable importance plot showing which predictors contribute most to reducing prediction error

The variable importance plot reveals which features drive remittance predictions in this non-linear model. Predictors at the top have the strongest influence.

------------------------------------------------------------------------

## 10.7 Model 4: K-Nearest Neighbors (KNN)

KNN predicts remittances by averaging the values of the K most similar country-year observations.

For each observation, finds the K closest observations in feature space (measured by Euclidean distance) and averages their remittance values. K is tunable.

KNN requires normalized features since it's distance-based. All predictors must be on the same scale.
...
Creates lagged variables respecting country groupings (can't use one country's previous year to predict another country's current year), then removes any remaining missing values.
...
Creates 10-fold CV with 5 repeats (50 total model fits) for robust RMSE estimates.
...
Sets up KNN to test 10 different K values ranging from 1 to 99. Small K = flexible but may overfit. Large K = smooth but may underfit.
...
Tests each K value across all cross-validation folds to find optimal number of neighbors.
...
Full results for all K values tested - Top 5 best-performing K values - Visual plot showing how RMSE changes with K (helps identify optimal flexibility)
...
Selects the K value with lowest cross-validated RMSE and finalizes the workflow.

------------------------------------------------------------------------

## 10.8 Model Comparison Using Cross-Validation
```{r}
## Compare all models using CV metrics only
model_comparison <- tibble(
  ...
  CV_RMSE = c(
    ...
    # Ridge - get RMSE at best penalty
    collect_metrics(ridge_res) |> 
      filter(.metric == "rmse") |> 
      inner_join(best_ridge, by = "penalty") |> 
      pull(mean),
    
    # LASSO - get RMSE at best penalty
    collect_metrics(lasso_res) |> 
      filter(.metric == "rmse") |> 
      inner_join(best_lasso, by = "penalty") |> 
      pull(mean),
...
This table compares all five models using **cross-validation metrics only**. The model with the lowest CV_RMSE performed best during training.

------------------------------------------------------------------------

## 10.9 Final Model Evaluation on Test Set

Based on cross-validation results, we now evaluate the best model on the held-out test set.
```{r}
## Identify the winner
...
best_workflow <- case_when(
  best_model_name == "OLS" ~ lm_workflow,
  best_model_name == "Ridge" ~ final_ridge_wf,
  best_model_name == "LASSO" ~ final_lasso_wf,
  best_model_name == "Random Forest" ~ final_rf_wf,
  best_model_name == "KNN" ~ final_knn_wf
)

## Evaluate ONLY the best model on test set using original split
final_test_results <- best_workflow |> 
  last_fit(remit_split)
...
final_test_results |>
  ...
  labs(
    title = paste("Test Set Performance:", best_model_name),
    subtitle = "Points closer to diagonal line = better predictions",
    x = "Actual Remittances (% GDP)",
    y = "Predicted Remittances (% GDP)"
  ) +
  theme_minimal()
```
...
tibble(
  ...
  CV_RMSE = model_comparison |> slice(1) |> pull(CV_RMSE),
  Test_RMSE = final_test_results |> collect_metrics() |> 
    filter(.metric == "rmse") |> pull(.estimate),
  CV_RSQ = model_comparison |> slice(1) |> pull(CV_RSQ),
  Test_RSQ = final_test_results |> collect_metrics() |> 
    filter(.metric == "rsq") |> pull(.estimate)
) |>
  mutate(across(where(is.numeric), ~round(., 3)))
```

This final table shows how our chosen model performs on completely unseen test data, confirming whether the cross-validation estimates were accurate.
