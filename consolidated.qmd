...
**I**The variable importance plot reveals which features drive remittance predictions in this non-linear model. Predictors at the top have the strongest influence.
...
Full results for all K values tested - Top 5 best-performing K values - V

isual plot showing how RMSE changes with K (helps identify optimal flexibility)
...
**What this does:** Selects the K value with lowest cross-validated RMSE and finalizes the workflow.

Now we evaluate our best KNN model on the held-out test set to get an unbiased estimate of performance on new data.
...
Applies the exact same transformations to test data as training data (lagged variables, variable selection, remove NAs).
...
Creates a proper train/test split object that trains the model on training data and evaluates on test data, ensuring no data leakage.
...
KNN predicts a country's remittances by finding the 11 most similar country-years in the data and averaging their remittance values.

We tested different numbers of neighbors (K) and found that using 11 neighbors works best.

The model performs well with an R² of 0.82, meaning it correctly predicts about 82% of the variation in remittances.

On average, the model's predictions are off by about 2.6 percentage points of GDP.
